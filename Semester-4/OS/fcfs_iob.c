...
int dispatcher(int time){
    int index = -1,max = 999;
    for(int i =0;i<n;i++){
    if(rl[i].status != 2 && rl[i].at <= time && rl[i].at < max){max = rl[i].at; index = i;}}
    return index;
}
void main(){
    ...
    for(i=0;i<n;i++){
        printf("Enter AT & BT & IO & BT2 of Process %d :",i+1); scanf("%d%d%d%d",&rl[i].at,&rl[i].bt,&rl[i].io,&rl[i].bt2);
        rl[i].status = 0; rl[i].ato = rl[i].at; rl[i].bto = rl[i].bt;
    }
    i = 0; cur_time =0;
    while(i < n){
