...
int dispatcher(int time){
    int index = -1,max = 999;
    for(int i=0;i<n;i++){
        if(rl[i].status != 1 && rl[i].at <= time && rl[i].bt < max){max = rl[i].bt; index = i;}
    }
    return index;
}
void main(){
    ...
    for(i=0;i<n;i++){
        printf("Enter Arrival & Burst time %d :",i+1); scanf("%d%d",&rl[i].at,&rl[i].bt);
        rl[i].status = 0; rl[i].bt1 = rl[i].bt;
    }
